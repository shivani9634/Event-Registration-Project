class DiscountCodesController < ApplicationController
  before_action :set_discount_code, only: [ :update, :destroy ]
  ...
  def destroy
    if @discount_code.destroy
      render json: { message: "Discount code successfully deleted." }, status: :ok
    else
      render json: { errors: "Failed to delete discount code." }, status: :unprocessable_entity
    end
  end
